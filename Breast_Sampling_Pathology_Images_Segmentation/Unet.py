...
TRAIN_IMAGE_PATH = r"D:\GitHub\Deep_Learning_in_Medical_Imaging\Breast_Sampling_Pathology_Images_Segmentation\Dataset\InputsTrain"
TRAIN_MASK_PATH = r"D:\GitHub\Deep_Learning_in_Medical_Imaging\Breast_Sampling_Pathology_Images_Segmentation\Dataset\MasksTrain"
TEST_IMAGE_PATH = r"D:\GitHub\Deep_Learning_in_Medical_Imaging\Breast_Sampling_Pathology_Images_Segmentation\Dataset\InputsTest"
TEST_MASK_PATH = r"D:\GitHub\Deep_Learning_in_Medical_Imaging\Breast_Sampling_Pathology_Images_Segmentation\Dataset\MasksTest"
...
''' Because train images and train masks have diffirent names in the tail or extention(ext), we should 
use data preprocessing. 
1. Syntax: os.path.basename(path) 
    path: A path-like object representing a file system path. 
    Return Type: This method returns a string value which represents the base name the specified path.
2. os.path.splitext() 
    To split the path name into a pair root and ext. Here, ext stands for extension and has the extension 
    portion of the specified path while root is everything except ext part.
...
def Data_Proprocessing_Train():
    ...
    # Now we define two new parameters for image and mask after resizing
    ...
    for mask_path in glob.glob('{}/*.TIF'.format(TRAIN_MASK_PATH)):
        ...
        # now we want to crop mask and image to only keep where we have the image
        # We crop the mask and image at the same time where the intensity value of 
        # mask is white or 255
        ...
        # resizes mask
        # To apply one hot encoding, we use np.expand_dims(): 
...
# we use np.squeeze because we use one hot encoding
print('Region_of_Interest_Mask')
imshow(np.squeeze(Train_Masks[0]))
plt.show()

# If we want to show all thses picture in one plot:
rows = 1
columns = 4
...
""" After the model is compiled, we have access to two values:
1. predicted value (float and should be converted to int)
2. real value
which can be used in our defined Intersection Over Unio_IOU """ 
